...
function showPhonePopup(numbers) {
    ...
    let buttonsHtml = numbers.map(num => {
        const cleanNum = num.replace(/\s/g, '');
        return `
            <a href="tel:${cleanNum}" class="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-emerald-50 transition border border-gray-200" onclick="closePhonePopup()">
    ...
    overlay.innerHTML = `
        <div class="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            <div class="p-4 border-b flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-800">Choisir un numéro</h3>
                <button onclick="closePhonePopup()" class="p-2 hover:bg-gray-100 rounded-lg">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    ${pharmacy.telephone ? `
                        <div class="flex items-center gap-1.5 mt-1">
                            <button onclick="handlePhoneClick('${pharmacy.telephone.replace(/'/g, "\\'")}', event)" class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 text-emerald-700 text-sm font-medium rounded-full hover:bg-emerald-100 transition cursor-pointer">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                                </svg>
                                ${pharmacy.telephone}
...
function showWelcomePopup(popup) {
    ...
    let imageHtml = '';
    if (popup.image_url) {
        imageHtml = `<div class="aspect-video"><img src="${popup.image_url}" alt="" class="w-full h-full object-cover"></div>`;
    }
    
    let warningHtml = '';
    if (popup.warning_text) {
        warningHtml = `
            <div class="mt-4 p-4 bg-red-50 border-2 border-red-400 rounded-xl text-center">
    ...
    overlay.innerHTML = `
        <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-fade-in">
            <div class="p-6 pb-3 text-center">
                <h2 class="text-xl font-bold text-gray-800">${popup.title}</h2>
            </div>
            ${imageHtml}
            <div class="p-6 text-center">
                ${popup.description ? `<p class="text-gray-600 mb-4">${popup.description}</p>` : ''}
                ${warningHtml}
                <button onclick="closeWelcomePopup(${popup.id})" class="mt-4 w-full py-3 bg-emerald-600 text-white font-semibold rounded-xl hover:bg-emerald-700 transition">
                    J'ai compris
                </button>
            </div>
        </div>
